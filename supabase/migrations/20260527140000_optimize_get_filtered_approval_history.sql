-- Optimize get_filtered_approval_history / _count: push the actor-scope filter
-- into the approval_history scan and add a composite index that matches the
-- DISTINCT ON sort key.
--
-- Problem: the latest_actions CTE in both functions scanned the entire
-- approval_history table on every call (no filter pushdown), then sorted by
-- (claim_id, acted_at DESC, id DESC). For users with large visible-claim sets
-- this hit the 8s statement_timeout in prod.
--
-- Fix:
--   1. Hoist role_scope + the set of claim ids the caller is allowed to see
--      into CTEs that are evaluated once per call.
--   2. Filter approval_history by that claim-id set INSIDE latest_actions, so
--      DISTINCT ON only walks rows belonging to visible claims.
--   3. New index approval_history (claim_id, acted_at DESC, id DESC) lets the
--      DISTINCT ON use an index scan instead of a full sort.
--
-- Function signatures and result columns are unchanged. No application code
-- change is required.

CREATE INDEX IF NOT EXISTS idx_approval_history_claim_acted_at_id
  ON public.approval_history (claim_id, acted_at DESC, id DESC);
