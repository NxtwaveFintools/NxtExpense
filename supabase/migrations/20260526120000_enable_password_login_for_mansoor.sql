...
-- Enables (or refreshes) email/password login for an existing auth user
-- (mansoor@nxtwave.co.in). Idempotent: on test it overwrites the previous
-- password and identity row; on production it freshly creates the email
-- identity alongside the existing SSO (azure) provider. This migration does
-- not create or delete auth users.
DO $$
DECLARE
  v_target_email constant text := 'mansoor@nxtwave.co.in';
  v_password constant text := 'hod@Nxtwave';
...
BEGIN
  ...
  RAISE NOTICE 'Password-enabled %', v_target_email;
END;
$$;
