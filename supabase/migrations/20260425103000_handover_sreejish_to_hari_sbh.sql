...
DECLARE
  ...
  v_tamil_nadu_state_id uuid;
  v_kerala_state_id uuid;
  v_approver_l1_role_id uuid;
  v_approver_l2_role_id uuid;
  v_admin_role_id uuid;
  v_admin_log_id uuid;
  v_previous_hari_designation_name text;
  v_previous_hari_l3_email text;
  v_replacement_reason text :=
    'System migration handover for Tamil Nadu and Kerala SBH coverage from Sreejish Mohanakumar to Hari Haran.';
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Handover aborted: old employee record for Sreejish not found.';
  END IF;

  SELECT *
  INTO v_new_employee
  FROM public.employees
  WHERE lower(employee_email) = 'hari.haran@nxtwave.co.in'
  FOR UPDATE;

  IF NOT FOUND THEN
    RAISE EXCEPTION 'Handover aborted: successor employee record for Hari not found.';
  END IF;
  ...
  SELECT id
  INTO v_tamil_nadu_state_id
  FROM public.states
  WHERE state_name = 'Tamil Nadu'
  LIMIT 1;

  SELECT id
  INTO v_kerala_state_id
  FROM public.states
  WHERE state_name = 'Kerala'
  LIMIT 1;
  ...
  IF v_tamil_nadu_state_id IS NULL OR v_kerala_state_id IS NULL THEN
    RAISE EXCEPTION 'Handover aborted: Tamil Nadu or Kerala state lookup is missing.';
  END IF;
  ...
  IF v_old_employee.designation_id <> v_sbh_designation_id THEN
    RAISE EXCEPTION 'Handover aborted: Sreejish is no longer mapped to SBH.';
  END IF;

  IF v_new_employee.designation_id NOT IN (v_abh_designation_id, v_sbh_designation_id) THEN
    RAISE EXCEPTION 'Handover aborted: Hari must currently be ABH or already promoted to SBH.';
  END IF;
  ...
  IF v_l2_ref_count > 0 OR v_l3_ref_count > 0 THEN
    RAISE EXCEPTION
      'Handover aborted: unexpected non-L1 references remain for Sreejish (L2: %, L3: %).',
      v_l2_ref_count,
      v_l3_ref_count;
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION
      'Handover aborted: employee_replacements already contains a conflicting mapping for Sreejish or Hari.';
  END IF;
  ...
  SELECT v_new_employee.id, v_tamil_nadu_state_id, true
  WHERE NOT EXISTS (
    SELECT 1
    FROM public.employee_states es
    WHERE es.employee_id = v_new_employee.id
      AND es.state_id = v_tamil_nadu_state_id
  );
  GET DIAGNOSTICS v_row_count = ROW_COUNT;
  v_mutation_count := v_mutation_count + v_row_count;

  INSERT INTO public.employee_states (employee_id, state_id, is_primary)
  SELECT v_new_employee.id, v_kerala_state_id, false
  WHERE NOT EXISTS (
    SELECT 1
    FROM public.employee_states es
    WHERE es.employee_id = v_new_employee.id
      AND es.state_id = v_kerala_state_id
  );
  GET DIAGNOSTICS v_row_count = ROW_COUNT;
  v_mutation_count := v_mutation_count + v_row_count;

  UPDATE public.employee_states
  SET is_primary = state_id = v_tamil_nadu_state_id
  WHERE employee_id = v_new_employee.id
    AND is_primary IS DISTINCT FROM (state_id = v_tamil_nadu_state_id);
  GET DIAGNOSTICS v_row_count = ROW_COUNT;
  v_mutation_count := v_mutation_count + v_row_count;
  ...
  IF v_mutation_count > 0 THEN
    ...
    VALUES (
      ...
      jsonb_build_object(
        ...
        'hari_previous_designation', coalesce(v_previous_hari_designation_name, 'UNKNOWN'),
        'hari_previous_designation_id', v_new_employee.designation_id,
        'hari_previous_l3_approver_id', v_new_employee.approval_employee_id_level_3,
        'hari_previous_l3_approver_email', coalesce(v_previous_hari_l3_email, 'NONE')
      ),
      jsonb_build_object(
        'new_designation_code', 'SBH',
        'new_primary_state', 'Tamil Nadu',
    ...
    VALUES (
      ...
      'employee_handover',
      format(
        'System handover migrated SBH approval ownership from %s to %s for Tamil Nadu and Kerala. Reassigned %s level-1 approval chains, routed Hari''s own SBH claims to %s as PM/HOD, and observed %s non-terminal claims pending at execution time. Previous Hari designation: %s. Previous Hari L3 approver: %s. Primary state enforced: Tamil Nadu.',
        v_old_employee.employee_email,
        v_new_employee.employee_email,
        v_reassigned_count,
        v_pm_employee.employee_email,
        v_pending_claim_count,
        coalesce(v_previous_hari_designation_name, 'UNKNOWN'),
        coalesce(v_previous_hari_l3_email, 'NONE')
      ),
      v_admin_employee.id
    );
  END IF;
