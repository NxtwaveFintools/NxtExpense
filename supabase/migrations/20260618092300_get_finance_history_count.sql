-- Phase 3a: bounded total-count RPC for the Approved History feed.
-- Replaces getFinanceHistoryTotalCount()'s chunked SAFE_IN_BATCH_SIZE counting.
-- Mirrors today's semantics exactly:
--   filters active  -> count of finance_actions for claims in finance_filtered_claim_ids(...)
--                      (the action-code/feed-date filters are NOT applied to the count,
--                       matching the current TS which counts ALL actions of matched claims)
--   no filters      -> count of all finance_actions
