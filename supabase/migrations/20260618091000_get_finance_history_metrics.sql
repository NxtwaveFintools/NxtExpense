-- Phase 2a — Approved History analytics RPC.
-- Ports getFinanceHistoryAnalytics() so NO claim-ID array is built in Node: the
-- claim scope comes from the Phase 1 resolver finance_filtered_claim_ids(), and the
-- action classification comes from finance_action_buckets() (single source of truth).
-- Mirrors get_finance_history_action_metrics, replacing its p_claim_ids uuid[] and
-- TS-passed action arrays with internal, set-based CTEs.
--
-- Hard constraint (same philosophy as the Phase 1 resolver): this function stays
-- declarative LANGUAGE sql — no plpgsql, no DECLARE, no array_agg of ids, no temp
-- tables. The CTEs (b, filtered, date_scoped, action_scope, approved, rejected,
-- scoped_actions) are set-based views the planner streams through; `filtered` is a
-- subquery against the resolver, never a materialized id collection.
...
as $$
  ...
  -- claim scope (resolver subquery, NOT a collected array)
  filtered as (
