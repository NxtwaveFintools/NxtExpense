...
DO $$
DECLARE
  v_old_email CONSTANT text := 'gajendra.k@nxtwave.co.in';
  v_new_email CONSTANT text := 'k.gajendra@nxtwave.co.in';
  v_new_domain text;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION
      'New email domain % is not active in allowed_email_domains.',
      v_new_domain;
  END IF;
