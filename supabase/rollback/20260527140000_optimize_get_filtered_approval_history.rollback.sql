-- Rollback for: 20260527140000_optimize_get_filtered_approval_history.sql
--
-- Reverts:
--   1. Drops the composite index added on approval_history.
--   2. Restores get_filtered_approval_history (17-arg overload) and
--      get_filtered_approval_history_count to their pre-optimization
--      definitions — the bodies established by 20260429080441_remote_schema.sql
--      and verified live against the test DB before the optimization migration
--      was authored.
--
-- The 14-arg shim of get_filtered_approval_history was not touched by the
-- forward migration, so it is not restored here.
--
-- CREATE OR REPLACE FUNCTION and DROP INDEX IF EXISTS are idempotent — this
-- rollback is safe to run multiple times.

DROP INDEX IF EXISTS public.idx_approval_history_claim_acted_at_id;
