-- Rollback for: 20260521163000_fix_state_city_rpc_return_type_casts.sql
--
-- That migration only redefined four RPCs via CREATE OR REPLACE FUNCTION
-- (no new tables/columns/triggers), so rolling it back simply restores those
-- functions to their prior definitions — the versions established by
-- 20260521162000_admin_state_city_management_and_historical_freeze.sql.
--
-- Note: the 20260521162000 migration in the repo already carries the same
-- ::text casts, so this rollback restores functionally identical definitions.
-- It is provided for migration-history consistency and is safe to run
-- multiple times (CREATE OR REPLACE is idempotent).
