-- Rollback for: 20260527160000_stream_paginated_approval_history.sql
--
-- Restores get_filtered_approval_history (17-param) to the
-- 20260527150000 version (LANGUAGE sql, paged_base MATERIALIZED approach).
--
-- NOTE: This restores a version that is still subject to cold-buffer
-- timeouts for large-scope users (e.g. Mansoor, 8 235 acted claims)
-- because it still scans 28 723 approval_history index rows and
-- 17 493 expense_claims rows before LIMIT is applied.
-- Only use this rollback if the plpgsql streaming function has a bug;
-- re-apply 20260527160000 with the fix instead.
...
AS $function$
  ...
  paged_base AS MATERIALIZED (
    ...
    WHERE
      ...
      AND (p_location_type IS NULL OR trim(p_location_type) = ''
           ...
           OR (lower(trim(p_location_type)) = 'base'
               AND EXISTS (SELECT 1 FROM public.work_locations wlb
                           WHERE wlb.id = c.work_location_id AND wlb.requires_outstation_details = false
      ...
      AND (p_cursor_acted_at IS NULL OR p_cursor_action_id IS NULL
  ...
  SELECT
    pb.action_id, pb.claim_id, pb.claim_number, pb.claim_date,
    pb.work_location, pb.total_amount, pb.claim_status, pb.claim_status_name,
    pb.claim_status_display_color, pb.owner_name,
    ...
    pb.action, pb.approval_level, pb.notes, pb.acted_at,
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE  ah_hod.claim_id = pb.claim_id AND to_status.approval_level = 3
      AND  to_status.is_approval = false AND to_status.is_rejection = false
      AND  to_status.is_terminal = false
    ORDER BY ah_hod.acted_at DESC LIMIT 1
  ) hod_event ON true
  LEFT JOIN LATERAL (
    SELECT fa.acted_at AS finance_approved_at
    FROM   public.finance_actions fa
    WHERE  fa.claim_id = pb.claim_id
      AND  EXISTS (SELECT 1 FROM payment_issued_actions pia
