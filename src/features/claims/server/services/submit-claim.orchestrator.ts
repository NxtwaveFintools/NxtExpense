...
import { mapClaimInsertErrorToUserMessage } from '@/features/claims/domain/claim-insert-errors'
import {
  type ClaimPayload,
  getClaimForDate,
  insertClaim,
  insertClaimItems,
  resolveInitialWorkflowState as resolveInitialWorkflowStateFromRepository,
  validateCitiesForSelectedState,
} from '@/features/claims/data/repositories/claims.repository'
import { supersedeRejectedClaim } from '@/features/claims/data/rpc/claim-lifecycle.rpc'
...
type SubmitClaimInput = Omit<ClaimFormValues, 'claimDate'> & {
...
type PersistedClaimContext = {
  expenseLocationId: string
  foodWithPrincipalsAmount: number | null
  hasIntercityScope: boolean
  hasIntracityScope: boolean
  ...
  resolvedBaseLocationDayType: ResolvedBaseLocationDayType | null
  totalAmount: number
...
function buildClaimPayload(
  ...
  return {
    ...
    foodWithPrincipalsAmount: isOutstation ? foodWithPrincipalsAmount : null,
  }
}
...
async function persistClaim(
  ...
  try {
    const claim = await insertClaim(
      supabase,
      buildClaimPayload(employeeId, designationId, context)
    )
...
export async function submitClaimOrchestrator(
  input: SubmitClaimInput
  ...
  if (wlFlags.requires_outstation_details) {
    ...
    if (hasIntercityTravel && !input.toCityId) {
      return {
        ok: false,
        error: 'To city is required for outstation travel.',
      }
    }
  ...
  if (
    ...
    const yearMonth = input.claimDate.iso.slice(0, 7)
  ...
  const persistContext: PersistedClaimContext = {
    expenseLocationId: expenseLocation.id,
    foodWithPrincipalsAmount: input.foodWithPrincipalsAmount ?? null,
    hasIntercityScope,
    hasIntracityScope,
    hasOwnVehicle,
    initialWorkflowState,
  ...
  try {
    if (existingClaim && existingClaim.is_rejection) {
      ...
      await supersedeRejectedClaim(supabase, existingClaim.id)
    }
